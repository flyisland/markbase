...
- The generated docsify entry HTML owns frontend-only internal-link adaptation, resource URL normalization, callout UI upgrades, the unified left-sidebar `Outline` / `Properties` / `Links` tab rail, note-only metadata tab eligibility for `.md` routes, docsify-outline-to-tab integration, sidebar note/base link adaptation into docsify hash routes, and embedded metadata such as the generating `markbase` version, homepage, and git commit/time; the backend web contract remains Markdown, metadata JSON, and resource bytes rather than callout-specific HTML.
