...
- The generated docsify entry HTML owns frontend-only internal-link adaptation, resource URL normalization, callout UI upgrades, and embedded metadata such as the generating `markbase` version, homepage, and git commit/time; the backend web contract remains Markdown, metadata JSON, and resource bytes rather than callout-specific HTML.
