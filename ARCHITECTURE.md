...
- The generated docsify entry HTML owns frontend-only internal-link adaptation, resource URL normalization, callout UI upgrades, note-only metadata sidebar tabs for eligible `.md` routes, sidebar-local scroll behavior, sidebar note/base link adaptation into docsify hash routes, and embedded metadata such as the generating `markbase` version, homepage, and git commit/time; the backend web contract remains Markdown, metadata JSON, and resource bytes rather than callout-specific HTML.
