...
These rules are always in effect. They override everything else.

### Multi-endpoint Sync

The vault has three concurrent writers: local Obsidian, local Bot, and remote Bot — all pushing to `main`. Remote may have new commits at any time.
...
| User says `commit`      | `git pull --rebase` → `git commit -m "<agent-generated message>"`                                    |
| User says `commit push` | `git pull --rebase` → `git commit -m "<agent-generated message>"` → `git push`                       |
| `git push` fails        | `git pull --rebase` → `git push` once more                                                           |
| Push still fails        | Stop. Tell the user — manual intervention required.                                                  |
| Conflict at any point   | Stop. Ask the user how to resolve. If unresolvable → `git rebase --abort` to return to a safe state. |

**Core principle: remote content must never be overwritten or lost.**

### Commit Authorization

- **Never commit without explicit user instruction.** Wait for `commit` or `commit push`.
- **Never `--force` push.** Never `--amend`. No exceptions.
- Commit messages are generated by the agent based on the changes made.
