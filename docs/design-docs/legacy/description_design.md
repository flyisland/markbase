...
补充说明：在模板语境中，`description` 存在三个不同层次：`_schema.description`（模板路由提示）、`_schema.properties.description`（实例字段的 schema 定义）以及 outer frontmatter `description`（实例 note 的真实摘要字段）。三者不能互相替代。更完整定义见 `docs/design-docs/legacy/template_schema.md`。
