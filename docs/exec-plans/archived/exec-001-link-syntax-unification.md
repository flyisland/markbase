...
- [x] task-0001: 建立共享 link/embed 解析器，替代多处直接 regex 解释
- [x] task-0002: 将 extractor 和 verifier 切换到共享解析语义

### Phase 2: 写路径与渲染路径

- [x] task-0003: 将 renamer 切换为基于 token span 的精确改写
- [x] task-0004: 为 renderer 增加 `.base#View` 支持，并与共享解析保持一致

### Phase 3: 合约与回归

- [x] task-0005: 同步设计文档、README、Agent 入口文档，并补齐跨模块回归测试
...
- `task-0001` 决定共享解析层 API，是后续所有任务的前置条件
- `task-0003` 和 `task-0004` 都依赖 `src/link_syntax.rs`，并且都会改 `tests/cli_note.rs`
- `task-0005` 必须在实现完成后再同步最终对外合约
...
原因：这些文件实际承担的是设计说明而非验证型 spec 职责。为避免与新的 `specs/*.md` 任务合约体系混淆，旧文件迁移到 `docs/references/legacy-designs/`，当前任务体系改为 `docs/exec-plans/` + `specs/active/`。

## Progress Notes

- 2026-03-14: 建立 `exec-001` 和配套 `task-0001` 至 `task-0005`，作为后续实现与验收入口
- 2026-03-14: 归档旧 `spec/*.md` 到 `docs/references/legacy-designs/`，保留历史设计上下文
...
4. `docs/design-docs/implemented/design-001-links-and-embeds.md` 与实现一致，不再把这些行为标成“已知限制”
