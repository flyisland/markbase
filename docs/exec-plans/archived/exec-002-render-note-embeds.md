...
- [x] task-0006: 为 renderer 增加 whole-note embed 展开，明确只输出 body 且 inline 按块拆行

### Phase 2: 递归与循环保护

- [x] task-0007: 为 note embed render 增加递归展开和 cycle guard

### Phase 3: 对外合约与回归

- [x] task-0008: 同步 README 与 render 回归测试，使实现、文档和验收保持一致
...
- `task-0006` 定义 note embed 的基础输出形态，是后续递归行为的前置条件
- `task-0007` 依赖 `task-0006` 已经建立 note embed 的 render 入口，否则 cycle guard 没有稳定挂载点
- `task-0008` 必须在实现完成后同步 README 和最终测试断言，避免文档提前锁死尚未收敛的实现细节
...
- 2026-03-14: 建立 `exec-002` 与配套 `task-0006` 至 `task-0008`，作为 note embed render 功能的实现入口
