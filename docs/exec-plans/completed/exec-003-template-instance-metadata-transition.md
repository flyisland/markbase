...
- [x] task-010: 将 `note verify` 从 outer frontmatter literal-match 语义切换到 `_schema.required` / `_schema.properties` 语义，并落实 stable identity rule

### Phase 3: 文档收口与兼容清理

- [x] task-011: 将 patch 设计并回正式文档，更新 README，并明确兼容行为的最终状态
...
- `task-009` 定义新的模板读模型和实例物化路径，是 `task-010` 的前置条件
- `task-010` 必须在 `task-009` 稳定后再切换 verifier，否则创建语义和验证语义会短暂分叉
- `task-011` 必须在实现收敛后再更新正式对外合同，避免过早锁死兼容细节
