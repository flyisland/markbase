...
mdb query "name == '绿米' and type == 'company'" -o json

# 1b. 按 aliases 匹配（frontmatter list 字段，has() 支持）
mdb query "has(aliases, '绿米联合') and type == 'company'" -o json
```
...
Step 1  执行 mdb template describe <模板名>（mdb new 之前）
...
4. **预读模板：** 执行 `mdb template describe <模板名>`，获取 `_schema`（含 `filename`）和所有正文指令。
...
3. **查模板：** 执行 `mdb template describe <模板名>`，获取全部正文指令。
4. **逐章扫描，按指令执行：**
