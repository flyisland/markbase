# MKS (Markdown Knowledge Schema) v1.5

**Status:** Stable / Production Ready  
**Date:** 2026-02-26  
**Target System:** mdb CLI, OpenClaw Agent  
**Changelog:** v1.4 → v1.5 见文末

---

## 1. 核心理念与流程 (Core Philosophy & Workflow)

MKS v1.5 是连接非结构化对话流与结构化知识库的协议。它不仅仅定义数据格式，更定义了知识的全生命周期管理流程：
...
位于 Frontmatter 的 `_schema` 属性下。负责数据库层面的强类型约束。**`_schema` 属性仅存在于模板文件中，实例文件不含此属性。**
...
_schema:
  ...
  # [2. 模板映射]
  # 必填。定义此模板对应的实体类型。实例文件通过 Frontmatter 中的 `type` 字段
  # 反查此模板，从而在沉淀阶段获取正文指令。两者必须严格一致。
  type: string

  # [3. 沉淀策略]
  # 默认为 false。允许保留 Schema 未定义的额外字段（兼容 Obsidian 插件数据）。
  # strict: false 时，Agent 对未定义字段采取「只读透传（read-only passthrough）」策略：
  # 读取其内容纳入上下文，但不主动写入或修改。
  strict: boolean
...
### 1.2 属性定义 (Schema Object)

| 属性 Key          | 类型   | 描述                                    | Obsidian UI 映射    |
| ----------------- | ------ | --------------------------------------- | ------------------- |
| **`type`**        | string | `string`, `number`, `boolean`, `array`  | 基础数据类型        |
| **`format`**      | string | `link`（双链）, `date`, `time`          | 特殊格式约束        |
| **`target`**      | string | 实体类型约束（仅当 `format: link`）     | **实体对齐核心**    |
| **`enum`**        | array  | 预设值列表                              | 下拉菜单 / 幻觉抑制 |
| **`description`** | string | 字段填写的 Prompt 指引                  | -                   |

---

## Part II: 血肉定义 (Body Directives)

正文指令**只存在于模板文件**中，不随实例文件复制。Agent 在沉淀阶段通过实例文件的 `type` 字段反查对应模板，获取指令后操作实例文件正文。
...
当 Agent 在信息流中识别到一个应为 `format: link` 的实体，但**无法立即确认其对应的实体文件**时（实体对齐流程返回不确定结果，或需等待人工确认），使用悬空引用语法占位：

```
[?[David Chen]]
```

区别于已确认的 `[[David Chen]]` 双链。悬空引用表示「已识别但待对齐的实体」，供后续人工或 Agent 批量处理。

**解除悬空引用的流程：**
1. 人工（或 Agent）执行实体对齐，确认目标实体文件。
2. 将文件中的 `[?[David Chen]]` 替换为 `[[David Chen]]`。
3. 若需要，同步将原始文本（如「David」）添加为目标实体的 `aliases`。

---

## Part III: 实体对齐算法 (Entity Alignment Algorithm)

当 Agent 处理 `format: link` 字段，或在沉淀阶段识别到人名/实体名时，执行以下流程。
...
# 1b. 按 aliases 匹配（frontmatter 数组，has() 支持）
mdb query "has(note.aliases, '绿米联合') and note.type == 'company'" -o json
```

两步均无结果时，进入步骤 2 的「零结果」分支。
...
用户指定别名关联到已有实体后，Agent 修改该实体文件的 Frontmatter：
...
修改完成后，重新执行 `mdb index` 使新 alias 生效。

---

## Part IV: 文件创建流程 (File Creation Workflow)

实例文件**必须通过 `mdb new` 创建**，禁止直接复制模板文件，以确保模板中的 `_schema` 定义和正文指令不污染实例文件。

### 完整流程

```
Step 1  Agent 读取模板文件
        ↓ 记住：_schema（required、properties）
        ↓ 记住：各章节的 [Fill] / [Update] 指令内容
        ↓ 此步骤在 mdb new 之前完成，Agent 在上下文中保留全部指令

Step 2  mdb new <实例名> --template <type>
...
**文件路径：** `templates/company.md`

```markdown
---
# [MKS v1.5 Template Definition]
_schema:
  description: >-
    标准客户档案模版。
...
3. **预读模板：** 读取 `meeting_log` 模板，记住所有指令（为后续 `mdb new` 做准备）。
...
1. 读取 `meeting_log` 的 `properties`，找到所有 `format: link` 字段。
2. 对每个字段，按 Part III 算法依次执行：name 精确匹配 → aliases 匹配 → 结果处理。
3. 唯一命中则写入双链；多结果则推断或询问；零结果则写入悬空引用。
...
2. **加载实例：** 读取 `company/绿米.md`，获取 `type: company`。
3. **查模板：** 反查 `_schema.type: company` 的模板文件，加载全部正文指令。
4. **逐章扫描，按指令执行：**
   ...
   | `## 4. 关键活动`  | `[Update]: Append`    | 幂等检查通过后，在末尾追加：`- [[2026-02-26]] [Visit] 讨论私有化部署 → [[2026-02-26_绿米拜访]]` |
...
| # | 变更内容 |
|---|----------|
| 1 | 新增 **Part IV 文件创建流程**，明确必须使用 `mdb new` 创建实例文件；定义 Agent 预读模板→创建骨架→填充的四步流程 |
| 2 | 新增 **悬空引用语法 `[?[...]]`**（Part II §2.4），定义占位、解除、alias 写回的完整流程；实体对齐零结果时写悬空引用而非阻塞 |
| 3 | `## 2. 组织架构与关键人` 从「人工维护」改为 **`[Update]: Accumulate`**，Agent 参与沉淀；人员条目区分双链与悬空引用，支持职务变动追加 |
| 4 | 实体对齐查询从 `mdb search`（不存在的命令）**修正为 `mdb query`**，给出 name 精确匹配和 aliases `has()` 匹配的具体命令 |
| 5 | 明确 alias 写入后需执行 `mdb index` 重新索引 |
| 6 | 模板范例同步更新，补全 `## 2` 章节指令，并统一各章节的格式规范说明 |